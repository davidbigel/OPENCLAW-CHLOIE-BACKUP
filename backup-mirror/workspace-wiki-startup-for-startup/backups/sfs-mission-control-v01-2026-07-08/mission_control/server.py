...
class PublicHandler(HandlerBase):
    ...
    def _proxy_to_private(self, method: str, path: str, body: dict[str, Any] | None) -> None:
        ...
        timeout_seconds = max(self.config.snoracle_timeout_seconds + 90, 180)
        conn = http.client.HTTPConnection(self.config.private_host, self.config.private_port, timeout=timeout_seconds)
...
class PrivateHandler(HandlerBase):
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/ask":
            ...
            self.send_json(200, self.service.ask(question, sources, client_request_id=client_request_id))
            return
        if parsed.path.startswith("/api/asks/") and parsed.path.endswith("/cancel"):
            client_request_id = parsed.path.split("/")[3]
            self.send_json(200, self.service.cancel_request(client_request_id))
            return
        ...
        if parsed.path.startswith("/api/questions/") and parsed.path.endswith("/rerun"):
            ...
            client_request_id = str(body.get("client_request_id") or "").strip() or None
            self.send_json(200, self.service.rerun(question_id, sources, client_request_id=client_request_id))
            return
...
instrument_class_methods(HandlerBase, LOGGER)
instrument_class_methods(PublicHandler, LOGGER)
instrument_class_methods(PrivateHandler, LOGGER)
