...
def load_config() -> Config:
    ...
    runtime_root = _resolve_runtime_root(app_root, workspace_root)
    ...
    snoracle_timeout_seconds = int(os.environ.get("MC_SNORACLE_TIMEOUT_SECONDS", "600"))
...
def _resolve_runtime_root(app_root: Path, workspace_root: Path) -> Path:
    configured = str(os.environ.get("MC_RUNTIME_ROOT") or "").strip()
    if configured:
        return Path(configured).expanduser().resolve()
    runtime_root = (app_root / "mission_control" / "logs").resolve()
    legacy_root = (workspace_root / "kb" / "mission-control").resolve()
    if not runtime_root.exists() and legacy_root.exists():
        runtime_root.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(legacy_root), str(runtime_root))
    return runtime_root
