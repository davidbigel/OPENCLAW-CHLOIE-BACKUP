...
class MissionControlService:
    ...
    def ask(self, question: str, sources: dict[str, bool], client_request_id: str | None = None) -> dict[str, Any]:
        ...
        self._bind_client_request(client_request_id, question_id)
        try:
            return self._run_question(question_id)
        finally:
            self._unbind_client_request(client_request_id, question_id)

    def rerun(self, question_id: str, sources: dict[str, bool] | None = None, client_request_id: str | None = None) -> dict[str, Any]:
        row = self.store.get_question_row(question_id)
        if not row:
            return {"status": "not_found", "message": "question not found"}
        selected = self._normalize_sources(sources or {
            "wikillm": bool(row.get("selected_wikillm")),
            "obsidian": bool(row.get("selected_obsidian")),
            "raw": bool(row.get("selected_raw")),
        })
        return self.ask(row["question"], selected, client_request_id=client_request_id)
    ...
    def cancel_request(self, client_request_id: str) -> dict[str, Any]:
        question_id = self._question_id_for_client_request(client_request_id)
        if not question_id:
            return {"status": "not_found", "client_request_id": client_request_id}
        result = self.cancel(question_id)
    ...
    def _run_question(self, question_id: str) -> dict[str, Any]:
        if not self._busy_lock.acquire(blocking=False):
            self.store.fail_question(question_id, "Snoracle became busy before processing could start")
            return self.store.get_question_response(question_id) or {"question_id": question_id, "status": "failed"}
        ...
        try:
            ...
            if not self.store.mark_question_running(question_id):
                return self.store.get_question_response(question_id) or {"question_id": question_id, "status": "cancelled"}
            if self.store.is_cancelled(question_id):
                return self.store.get_question_response(question_id) or {"question_id": question_id, "status": "cancelled"}
            selected = self._normalize_sources({
                "wikillm": bool(row.get("selected_wikillm")),
                "obsidian": bool(row.get("selected_obsidian")),
                "raw": bool(row.get("selected_raw")),
            })
    ...
    def _normalize_sources(self, sources: dict[str, bool] | None) -> dict[str, bool]:
        return {
    ...
    def _bind_client_request(self, client_request_id: str | None, question_id: str) -> None:
        if not client_request_id:
            return
        with self._request_lock:
            self._client_requests[client_request_id] = question_id

    def _unbind_client_request(self, client_request_id: str | None, question_id: str) -> None:
        if not client_request_id:
            return
        with self._request_lock:
            if self._client_requests.get(client_request_id) == question_id:
                self._client_requests.pop(client_request_id, None)

    def _question_id_for_client_request(self, client_request_id: str) -> str | None:
        with self._request_lock:
            return self._client_requests.get(client_request_id)
...
instrument_class_methods(MissionControlService)
