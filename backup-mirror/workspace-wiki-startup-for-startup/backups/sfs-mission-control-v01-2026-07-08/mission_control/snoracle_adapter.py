...
def synthesize_answer(
    ...
    *,
    ...
    if timeout_hit:
        ...
        payload["error"] = f"OpenClaw turn timed out after {config.snoracle_timeout_seconds} seconds"
...
def _build_action_prompt(question_id: str, question: str, selected_sources: dict[str, bool]) -> str:
    ...
    return (
        ...
        "Work fast. Give a concise direct answer and avoid exhaustive corpus sweeps unless the user explicitly asks for one.\n"
        "If WikiLLM is selected, start there. Use raw transcripts only when you need direct transcript evidence or WikiLLM is insufficient.\n"
        "If a selected source has no useful evidence, mark it no_useful_evidence quickly instead of digging indefinitely.\n"
...
def _payload_from_raw(
    ...
    wrapper = _extract_openclaw_wrapper(raw_response)
    if wrapper is not None and _wrapper_is_timeout(wrapper):
        error = _wrapper_error_message(wrapper)
        return (
            _fallback_payload(
                question_id,
                error,
                selected_sources,
                "failed",
                "OpenClaw timed out before returning Mission Control JSON",
            ),
            "timeout",
        )
    try:
...
def _extract_openclaw_wrapper(text: str) -> dict[str, Any] | None:
    stripped = text.strip()
    if not stripped:
        return None
    for candidate in _json_candidates(stripped):
        try:
            ...
            continue
        if isinstance(parsed, dict) and any(key in parsed for key in ("runId", "status", "result")):
            return parsed
    return None


def _wrapper_is_timeout(wrapper: dict[str, Any]) -> bool:
    status = str(wrapper.get("status") or "").lower()
    summary = str(wrapper.get("summary") or "").lower()
    stop_reason = str(wrapper.get("stopReason") or "").lower()
    return status == "timeout" or summary == "aborted" or stop_reason == "rpc"


def _wrapper_error_message(wrapper: dict[str, Any]) -> str:
    ...
    return "Request timed out before a response was generated."
