...
from mission_control.config import Config, _resolve_runtime_root, load_config
...
def temp_config(root: Path) -> Config:
    app_root = APP_ROOT
    runtime_root = root / "sfs-mission-control" / "mission_control" / "logs"
    return Config(
        app_root=app_root,
        workspace_root=root,
...
class CoreTests(unittest.TestCase):
    ...
    def test_runtime_secret_generation_and_default_openclaw_mode(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict("os.environ", {"MC_WORKSPACE_ROOT": tmp, "MC_RUNTIME_ROOT": str(runtime_root)}, clear=True):
                config = load_config()
                ...
                self.assertEqual(config.snoracle_timeout_seconds, 600)
                self.assertTrue((runtime_root / ".secrets.json").exists())
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with patch.dict(
                "os.environ",
                {
                    ...
                    "MC_RUNTIME_ROOT": str(Path(tmp) / "runtime"),
    ...
    def test_runtime_root_moves_legacy_workspace_dir(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            app_root = Path(tmp) / "app"
            workspace_root = Path(tmp) / "workspace"
            legacy_root = workspace_root / "kb" / "mission-control"
            app_root.mkdir(parents=True)
            legacy_root.mkdir(parents=True)
            (legacy_root / "sentinel.txt").write_text("moved", encoding="utf-8")
            resolved = _resolve_runtime_root(app_root, workspace_root)
            self.assertEqual(resolved, (app_root / "mission_control" / "logs").resolve())
            self.assertTrue((resolved / "sentinel.txt").exists())
    ...
    def test_timeout_wrapper_becomes_failed_payload(self) -> None:
        wrapped = json_dumps_for_test(
            {
                "runId": "r1",
                "status": "timeout",
                "summary": "aborted",
        ...
        payload, parse_status = _payload_from_raw(
            "q_1",
            wrapped,
            {"wikillm": True, "obsidian": False, "raw": True},
            0,
        ...
        self.assertEqual(parse_status, "timeout")
        self.assertEqual(payload["status"], "failed")
        self.assertIn("timed out", payload["summary_answer_markdown"])
    ...
    def test_static_assets_do_not_expose_gate_value(self) -> None:
        static_html = (APP_ROOT / "static" / "index.html").read_text(encoding="utf-8")
        app_js = (APP_ROOT / "static" / "app.js").read_text(encoding="utf-8")
        static_text = static_html + app_js
        self.assertNotIn("?p=", static_text)
        self.assertNotIn("3d20", static_text)
        self.assertNotIn('id="sourceObsidian" checked', static_html)
        self.assertNotIn("pollQuestion", app_js)
        self.assertNotIn("watchQuestion", app_js)
